  ...
  class UpdateDatabaseRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `VultrRuby::UpdateDatabaseRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `VultrRuby::UpdateDatabaseRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [region, plan, label, tag, vpc_id, maintenance_dow, maintenance_time, cluster_time_zone, trusted_ips, mysql_sql_modes, mysql_require_primary_key, mysql_slow_query_log, mysql_long_query_time, redis_eviction_policy].hash
    end
