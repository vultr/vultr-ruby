  ...
  class UpdateInstanceRequest
    ...
    def self.openapi_types
      {
        ...
        :'detach_private_network' => :'Array<String>',
        :'detach_vpc' => :'Array<String>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `VultrRuby::UpdateInstanceRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `VultrRuby::UpdateInstanceRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'detach_private_network')
        if (value = attributes[:'detach_private_network']).is_a?(Array)
          self.detach_private_network = value
        end
      end

      if attributes.key?(:'detach_vpc')
        if (value = attributes[:'detach_vpc']).is_a?(Array)
          self.detach_vpc = value
        end
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          detach_private_network == o.detach_private_network &&
          detach_vpc == o.detach_vpc &&
    ...
    def hash
      [app_id, image_id, backups, firewall_group_id, enable_ipv6, os_id, user_data, tag, plan, ddos_protection, attach_private_network, attach_vpc, attach_vpc2, detach_private_network, detach_vpc, detach_vpc2, enable_private_network, enable_vpc, enable_vpc2, label, tags, user_scheme].hash
    end
