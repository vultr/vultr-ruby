  ...
  class LoadbalancerFirewallRule
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `VultrRuby::LoadbalancerFirewallRule` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `VultrRuby::LoadbalancerFirewallRule`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@ip_type.nil? && @ip_type.to_s.length < 1
        invalid_properties.push('invalid value for "ip_type", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if !@ip_type.nil? && @ip_type.to_s.length < 1
      true
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] ip_type Value to be assigned
    def ip_type=(ip_type)
      if ip_type.nil?
        fail ArgumentError, 'ip_type cannot be nil'
      end

      if ip_type.to_s.length < 1
        fail ArgumentError, 'invalid value for "ip_type", the character length must be great than or equal to 1.'
      end
